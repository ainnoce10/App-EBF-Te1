...
const Technicians: React.FC<TechniciansProps> = ({ initialData = [] }) => {
  ...
  return (
    <div className="space-y-6 pb-20 relative">
      ...
      {/* MODAL RAPPORT VOCAL - FLOTTANT ET SANS FOND NOIR MASQUANT */}
      {showReportModal && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 pointer-events-none overflow-hidden">
           <div className="bg-white/95 backdrop-blur-xl w-full max-w-sm rounded-[3.5rem] p-8 shadow-[0_25px_100px_-20px_rgba(0,0,0,0.5)] relative border-b-[8px] border-orange-500 flex flex-col items-center animate-scale-in pointer-events-auto ring-1 ring-black/5">
              
              {/* Bouton fermeture */}
              <button 
                onClick={() => { setShowReportModal(false); setRecordingState('idle'); setAudioBlob(null); setAudioUrl(null); }} 
                className="absolute top-6 right-6 p-2 bg-gray-100 rounded-full text-gray-400 active:scale-90"
              >
                  <X size={20}/>
              </button>
              ...
              <div className="text-center mb-6 mt-4">
                  ...
                  <p className="text-orange-500 font-bold text-[10px] uppercase mt-1 tracking-widest flex items-center justify-center gap-2">
                    {activeInterventionForReport ? <><CheckCircle2 size={12}/> {activeInterventionForReport.client}</> : 'Nouveau Rapport'}
                  </p>
              </div>

              {/* ZONE CENTRALE */}
              <div className="relative flex flex-col items-center justify-center gap-6 py-4 w-full">
                  <div className="relative flex items-center justify-center">
                      ...
                      <button 
                        ...
                        className={`w-32 h-32 md:w-40 md:h-40 rounded-full flex flex-col items-center justify-center shadow-2xl transition-all active:scale-95 border-8 border-white
                          ${recordingState === 'recording' ? 'bg-red-500' : recordingState === 'review' ? 'bg-orange-500' : 'bg-gray-900'}`}
                      >
                          {recordingState === 'idle' && (
                              <div className="flex flex-col items-center text-white">
                                  <Mic size={40} strokeWidth={2.5}/>
                                  <span className="text-[8px] font-black uppercase mt-2 tracking-widest">Lancer</span>
                              </div>
                          )}
                          {recordingState === 'recording' && (
                              <div className="flex flex-col items-center text-white">
                                  <Square size={36} fill="white" />
                                  <span className="text-[8px] font-black uppercase mt-2 tracking-widest">Terminer</span>
                              </div>
                          )}
                  ...
                  <div className="text-center">
                      <p className="text-5xl font-black font-mono text-gray-950 tabular-nums tracking-tighter">
              ...
              {/* ACTIONS FINALES */}
              {recordingState === 'review' && (
                <div className="w-full space-y-3 mt-6 animate-slide-up">
                   {!activeInterventionForReport && (
                        <select className="w-full p-4 bg-gray-50 rounded-2xl font-black text-xs border-2 border-transparent focus:border-orange-500 outline-none appearance-none" value={formReport.client} onChange={(e) => setFormReport({...formReport, client: e.target.value})}>
      ...
      {showNewInterventionModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-2 bg-black/60 backdrop-blur-sm animate-fade-in">
           <div className="bg-white w-full max-w-2xl rounded-[3rem] p-6 md:p-10 shadow-2xl max-h-[92vh] overflow-y-auto custom-scrollbar">
              <div className="flex justify-between items-center mb-6">
                  <h3 className="text-xl font-black uppercase italic tracking-tight">Nouvelle Mission</h3>
                  <button onClick={() => setShowNewInterventionModal(false)} className="p-3 bg-gray-100 rounded-full text-gray-500 active:bg-red-50"><X size={20}/></button>
              </div>
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-fade-in">
              <div className="bg-white w-full max-w-xl rounded-[3rem] p-8 md:p-10 shadow-2xl relative animate-slide-up">
                  <div className="flex justify-between items-start mb-6">
                      ...
                      <button onClick={() => setViewIntervention(null)} className="p-3 bg-gray-100 rounded-full active:bg-gray-200"><X size={20} /></button>
                  </div>
                  <div className="space-y-4">
      ...
      {editIntervention && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 p-4 animate-fade-in">
           <div className="bg-white w-full max-w-md rounded-[3rem] p-8 shadow-2xl animate-scale-in">
               <div className="flex justify-between items-center mb-6">
                  <h3 className="text-lg font-black uppercase italic tracking-tighter">Modifier le Statut</h3>
               ...
               <div className="space-y-4">
                   ...
                   <button onClick={handleUpdateIntervention} disabled={isSaving} className="w-full py-4 bg-gray-950 text-white rounded-[1.5rem] font-black uppercase text-[10px] tracking-widest mt-4 shadow-xl active:scale-95 transition-all">
                     {isSaving ? <Loader2 className="animate-spin mx-auto" size={18} /> : 'Appliquer les modifications'}
                   </button>
               </div>
           </div>
        </div>
